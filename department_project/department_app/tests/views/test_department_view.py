...
class DepartmentTestCase(APITestCase):
    ...
    def test_create(self):
        ...
        response = self.client.post(reverse('department-list'), {'full_name': 'TEST'}, format='json')
        self.assertEqual(201, response.status_code)
        self.assertEqual(past_count + 1, Department.objects.count())
        self.assertTrue(Department.objects.filter(full_name='TEST').exists())

    def test_update(self):
        """Testing update a department"""
        response = self.client.put(reverse('department-detail', args=[1]), json.dumps({"full_name": "TEST"}),
                                   content_type='application/json')
        self.assertEqual(200, response.status_code)
        self.assertTrue(Department.objects.filter(id=1, full_name='TEST').exists())
    ...
    def test_create_empty(self):
        ...
        past_count = Department.objects.count()
        response = self.client.post(reverse('department-list'), {'full_name': ''}, format='json')
        self.assertEqual(400, response.status_code)
        self.assertEqual(past_count, Department.objects.count())
        self.assertFalse(Department.objects.filter(full_name='').exists())
    ...
    def test_create_long(self):
        ...
        past_count = Department.objects.count()
        response = self.client.post(reverse('department-list'), {'full_name': name}, format='json')
        self.assertEqual(400, response.status_code)
        self.assertEqual(past_count, Department.objects.count())
        self.assertFalse(Department.objects.filter(full_name=name).exists())
